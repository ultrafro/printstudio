...
export function PrintStudioApp() {
  ...
  }, [bridgeUrl, logActivity, sessionId]);
  ...
  return (
    <main className="grain min-h-screen px-4 py-4 text-slate-900 md:px-6">
      <div
        ref={workspaceRef}
        className={`mx-auto grid min-h-[calc(100vh-2rem)] max-w-[1680px] grid-cols-1 gap-4 rounded-[30px] border border-white/60 p-4 shadow-[0_28px_80px_rgba(15,23,42,0.12)] md:grid-cols-[360px_minmax(0,1fr)_360px] ${agentConnected ? "agent-glow" : ""}`}
        style={{ backgroundImage: "var(--hero)" }}
      >
        <section className="panel panel-strong rounded-[26px] p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="font-mono text-xs uppercase tracking-[0.28em] text-slate-500">
                PrintStudio
              </p>
              <h1 className="mt-2 text-4xl font-semibold leading-none text-slate-900">
                Agent-ready STL workbench
              </h1>
            </div>
            <div className="rounded-full border border-slate-200 bg-white/70 px-3 py-1 font-mono text-xs text-slate-600">
              {sessionId}
            </div>
          </div>

          <p className="mt-4 text-sm leading-6 text-slate-700">
            Design STL files in the browser, expose a live tool suite over a websocket,
            and close the verification loop with screenshots before exporting for the
            Bambu Lab P1S.
          </p>

          <div
            className={`mt-5 rounded-2xl border px-4 py-3 ${agentConnected ? "border-emerald-300 bg-emerald-50" : "border-slate-200 bg-white/70"}`}
          >
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-lg font-semibold text-slate-900">{agentName}</p>
              </div>
              <span
                className={`rounded-full px-3 py-1 font-mono text-xs ${agentConnected ? "bg-emerald-600 text-white" : "bg-slate-900 text-white"}`}
              >
          ...
          <label className="mt-5 block">
            <span className="mb-2 block font-mono text-xs uppercase tracking-[0.2em] text-slate-500">
              Bridge URL
            </span>
            <input
              value={bridgeUrl}
          ...
          <div className="mt-3 flex gap-3">
            <button
              ...
              className="flex-1 rounded-2xl bg-slate-950 px-4 py-3 text-sm font-medium text-white transition hover:bg-slate-800"
            >
              Connect To Claude Code / Agents
            </button>
            <button
              type="button"
              onClick={() =>
              ...
              className="rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-medium text-slate-900 transition hover:bg-slate-50"
            >
              Reset Tray
            </button>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <button
              ...
              className="rounded-2xl border border-sky-300 bg-sky-50 px-4 py-3 text-sm font-medium text-sky-900"
            >
              Export STL
            </button>
          </div>
          ...
          <label className="mt-5 block">
            ...
            <textarea
              ...
              className="min-h-28 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm leading-6 outline-none"
            />
          </label>
          ...
          <div className="mt-5">
            <div className="flex items-center justify-between">
              ...
              <p className="text-xs text-slate-500">{project.shapes.length} shapes</p>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2">
              {(["box", "roundedBox", "cylinder", "sphere", "text"] as const).map((primitive) => (
                <button
                  ...
                  className="rounded-2xl border border-slate-200 bg-white px-3 py-3 text-sm font-medium capitalize text-slate-900 transition hover:border-slate-300 hover:bg-slate-50"
                >
                  Add {primitive}
                </button>
              ))}
          ...
          <div className="mt-5">
            <label className="mb-2 block font-mono text-xs uppercase tracking-[0.2em] text-slate-500">
              Import STL
            </label>
            <input
              type="file"
            ...
            <div className="mt-5 rounded-[24px] border border-slate-200 bg-white p-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-2xl border border-slate-200 bg-slate-50 p-1"
                />
              </div>
        ...
        <section className="panel panel-strong rounded-[26px] p-4">
          <div className="flex h-full flex-col gap-4">
            <div className="flex items-center justify-between rounded-[24px] border border-white/70 bg-white/65 px-5 py-4">
              <div>
                <p className="font-mono text-xs uppercase tracking-[0.2em] text-slate-500">
                  Active Project
                </p>
              ...
              <div className="text-right">
                ...
                <p className="mt-1 text-sm font-medium text-slate-900">{displayedBridgeState}</p>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="rounded-[24px] border border-slate-200 bg-white p-4">
                ...
                <p className={`mt-2 text-2xl font-semibold ${printAnalysis.fitsP1S ? "text-emerald-700" : "text-amber-700"}`}>
              ...
              <div className="rounded-[24px] border border-slate-200 bg-white p-4">
                ...
                {exportArtifact ? (
                  <>
                    <p className="mt-2 text-lg font-semibold text-slate-900">{exportArtifact.filename}</p>
                    <a
                      href={exportArtifact.downloadUrl}
                      download={exportArtifact.filename}
                      className="mt-3 inline-flex rounded-full bg-slate-950 px-4 py-2 text-sm font-medium text-white"
                    >
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="rounded-full border border-slate-300 px-3 py-2 text-xs font-medium text-slate-900"
                  >
                    Capture Scene
                  </button>
                  <button
        ...
        <section className="panel panel-strong rounded-[26px] p-5">
          <div className="rounded-[24px] border border-slate-200 bg-white px-4 py-4">
            ...
            <div className="mt-3 space-y-3">
              {AGENT_TOOL_MANIFEST.map((tool) => (
                <div key={tool.name} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                  <div className="flex items-center justify-between gap-3">
                    <p className="font-medium text-slate-900">{tool.name}</p>
                    <span className="rounded-full bg-white px-2 py-1 font-mono text-[10px] uppercase tracking-[0.18em] text-slate-500">
                      {tool.category}
                    </span>
                  </div>
          ...
          <div className="mt-5 rounded-[24px] border border-slate-200 bg-white px-4 py-4">
            <div className="flex items-center justify-between">
              ...
              <p className="text-xs text-slate-500">{toolCalls.length} tracked</p>
            </div>
            <div className="mt-3 space-y-3">
              {toolCalls.length > 0 ? (
                toolCalls.map((call) => (
                  <div key={call.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <p className="font-medium text-slate-900">{call.tool}</p>
                      <span
                    ...
                    <p className="mt-2 break-all font-mono text-xs text-slate-500">{call.summary}</p>
                    {call.resultPreview ? (
                      <p className="mt-2 text-xs leading-5 text-slate-600">{call.resultPreview}</p>
                    ) : null}
                  </div>
                ))
              ) : (
          ...
          <div className="mt-5 rounded-[24px] border border-slate-200 bg-white px-4 py-4">
            <div className="flex items-center justify-between">
              ...
              <p className="text-xs text-slate-500">{activity.length} events</p>
            </div>
            <div className="mt-3 space-y-3">
              {activity.map((entry) => (
                <div key={entry.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                  ...
                  {entry.detail ? <p className="mt-2 text-sm text-slate-600">{entry.detail}</p> : null}
                </div>
              ))}
            </div>
          </div>
